...
def scrape_rightmove_page(url):
    ...
    try:
        ...
        for card in property_cards:
            try:
              ...
              def scrape_rightmove_page(url):
    """Scrape property listings from Rightmove search page using modern 2026 selectors"""
    properties = []
    
    # Updated headers to bypass basic security
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36',
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8',
        'Accept-Language': 'en-GB,en;q=0.9',
    ...
    try:
        ...
        # 1. Primary selector for modern Rightmove search results
        property_cards = soup.find_all('div', class_='l-searchResult')
        
        # 2. Fallback selector using data-test attributes
        if not property_cards:
            property_cards = soup.find_all('div', attrs={'data-test': re.compile(r'propertyCard-\d+')})
        ...
        for card in property_cards:
            try:
                # Find the link first to get the ID
                link_elem = card.find('a', class_='propertyCard-link') or card.find('a', href=True)
                if not link_elem: continue
                
                href = link_elem.get('href', '')
                # Extract ID from URL: /properties/12345678#/?channel=RES_BUY
                ...
                # Extract Bedrooms from the card text
                bedrooms = 0
